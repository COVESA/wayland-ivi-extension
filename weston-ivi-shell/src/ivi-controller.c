...
static void
surface_commited(struct wl_listener *listener, void *data)
{
    struct ivisurface *ivisurf = wl_container_of(listener, ivisurf, commited);
    (void)data;
               ...
               uint32_t id_surface)
{
    ...
    ivisurf->commited.notify = surface_commited;
    surface = lyt->surface_get_weston_surface(layout_surface);
    wl_signal_add(&surface->commit_signal, &ivisurf->commited);
...
surface_event_remove(struct wl_listener *listener, void *data)
{
    ...
    wl_list_remove(&ivisurf->commited.link);
    free(ivisurf);

...
destroy_ivicontroller_surface(struct wl_resource *resource)
{
    ...
    if (ctrl) {
        clear_notification_list(&ctrl->surface_notifications);
        ctrl->surface_resource = NULL;
    }
}
...
unbind_resource_controller(struct wl_resource *resource)
{
    ...
    if (controller->layer_resource) {
        clear_notification_list(&controller->layer_notifications);
        wl_resource_set_user_data(controller->layer_resource, NULL);
    }

    if (controller->surface_resource) {
        clear_notification_list(&controller->surface_notifications);
        wl_resource_set_user_data(controller->surface_resource, NULL);
    }
                             ...
                             uint32_t surface_id)
{
    ...
    ivi_manager_surface_send_configuration(ctrl->surface_resource,
                                           surface_id,
                                           surface->width,
                                           surface->height);
}
                   ...
                   uint32_t mask)
{
    if (mask & IVI_NOTIFICATION_OPACITY) {
        ivi_manager_surface_send_opacity(ctrl->surface_resource, surface_id, prop->opacity);
    }
    if (mask & IVI_NOTIFICATION_SOURCE_RECT) {
        ivi_manager_surface_send_source_rectangle(ctrl->surface_resource, surface_id,
            prop->source_x, prop->source_y,
            prop->source_width, prop->source_height);
    }
    if (mask & IVI_NOTIFICATION_DEST_RECT) {
        ivi_manager_surface_send_destination_rectangle(ctrl->surface_resource, surface_id,
            prop->dest_x, prop->dest_y,
            prop->dest_width, prop->dest_height);
    }
    if (mask & IVI_NOTIFICATION_ORIENTATION) {
        ivi_manager_surface_send_orientation(ctrl->surface_resource, surface_id, prop->orientation);
    }
    if (mask & IVI_NOTIFICATION_VISIBILITY) {
        ivi_manager_surface_send_visibility(ctrl->surface_resource, surface_id, prop->visibility);
    }
...
{
    if (mask & IVI_NOTIFICATION_OPACITY) {
        ivi_manager_layer_send_opacity(ctrl->layer_resource,
                                       layer_id,
                                       prop->opacity);
    }
    if (mask & IVI_NOTIFICATION_SOURCE_RECT) {
        ivi_manager_layer_send_source_rectangle(ctrl->layer_resource,
                                          layer_id,
                                          prop->source_x,
                                          prop->source_y,
                                          prop->source_width,
    ...
    if (mask & IVI_NOTIFICATION_DEST_RECT) {
        ivi_manager_layer_send_destination_rectangle(ctrl->layer_resource,
                                          layer_id,
                                          prop->dest_x,
                                          prop->dest_y,
                                          prop->dest_width,
    ...
    if (mask & IVI_NOTIFICATION_ORIENTATION) {
        ivi_manager_layer_send_orientation(ctrl->layer_resource,
                                           layer_id,
                                           prop->orientation);
    }
    if (mask & IVI_NOTIFICATION_VISIBILITY) {
        ivi_manager_layer_send_visibility(ctrl->layer_resource,
                                          layer_id,
                                          prop->visibility);
    }
}
                   ...
                   wl_fixed_t opacity)
{
    ...
    if (!layout_surface) {
        ivi_manager_surface_send_error(resource, surface_id,
                                     IVI_MANAGER_SURFACE_ERROR_NO_SURFACE,
                   ...
                   int32_t orientation)
{
    ...
    if (!layout_surface) {
        ivi_manager_surface_send_error(resource, surface_id,
                                     IVI_MANAGER_SURFACE_ERROR_NO_SURFACE,
                                     "surface_set_orientation: the surface with given id does not exist");
        return;
    }

    lyt->surface_set_orientation(layout_surface, (uint32_t)orientation);
}
                  ...
                  const char *filename)
{
    ...
    if (!width || !height || !stride) {
        ivi_manager_surface_send_error(resource, surface_id,
                                       IVI_MANAGER_SURFACE_ERROR_BAD_PARAM,
    ...
    if (result != IVI_SUCCEEDED) {
        ...
        free(buffer);
        ivi_manager_surface_send_error(resource, surface_id,
                                       IVI_MANAGER_SURFACE_ERROR_NOT_SUPPORTED,
                   ...
                   uint32_t surface_id)
{
    ...
    ivi_manager_surface_send_stats(ctrl->surface_resource, surface_id, 0, 0,
                                      0, pid, "");
}
...
static void
controller_surface_set_type(struct wl_client *client, struct wl_resource *resource,
...
convert_protocol_enum(int32_t param)
{
    ...
    if (param & IVI_MANAGER_PARAM_OPACITY)
        mask |= IVI_NOTIFICATION_OPACITY;

    if (param & IVI_MANAGER_PARAM_VISIBILITY)
        mask |= IVI_NOTIFICATION_VISIBILITY;

    if (param & IVI_MANAGER_PARAM_ORIENTATION)
        mask |= IVI_NOTIFICATION_ORIENTATION;

    if (param & IVI_MANAGER_PARAM_SIZE) {
...
static const
struct ivi_manager_surface_interface controller_surface_implementation = {
                    ...
                    uint32_t version, uint32_t id)
{
    ...
    controller->resource =
        wl_resource_create(client, &ivi_manager_interface, 1, id);
             ...
             uint32_t id_layer)
{
    ...
    wl_list_for_each(controller, &shell->list_controller, link) {
        if (controller->layer_resource)
            ivi_manager_layer_send_created(controller->layer_resource, id_layer);
    }
               ...
               uint32_t id_surface)
{
    ...
    wl_list_for_each(controller, &shell->list_controller, link) {
        if (controller->surface_resource)
            ivi_manager_surface_send_created(controller->surface_resource, id_surface);
    }
...
layer_event_remove(struct wl_listener *listener, void *data)
{
    ...
    wl_list_for_each(controller, &shell->list_controller, link) {
        if (controller->layer_resource)
            ivi_manager_layer_send_destroyed(controller->layer_resource, id_layer);
    }
}
...
surface_event_remove(struct wl_listener *listener, void *data)
{
    ...
    wl_list_for_each(controller, &shell->list_controller, link) {
        if (controller->surface_resource)
            ivi_manager_surface_send_destroyed(controller->surface_resource, id_surface);
    }
}
...
surface_event_configure(struct wl_listener *listener, void *data)
{
    ...
    if (ivisurf->type == IVI_MANAGER_SURFACE_TYPE_DESKTOP) {
...
setup_ivi_controller_server(struct weston_compositor *compositor,
                            struct ivishell *shell)
{
    if (wl_global_create(compositor->wl_display, &ivi_manager_interface, 1,
